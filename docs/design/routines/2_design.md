...
`crates/orbit-automation/src/routines/`. The scheduler itself is registry-neutral and
host-neutral — it reads neither registry file, and reaches its workspaces only through the
`AutomationHost` port Core implements [ORB-12262]. There is no hub snapshot, satellite cache, fleet
