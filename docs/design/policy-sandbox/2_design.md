...
- `Sandbox::spawn(req)` creates the child after validation. The default launches through `process::spawn`; activity-scoped `proc.spawn` overrides it.
...
Activity-scoped `proc.spawn` still rejects explicit path arguments (including `--key=path` and `git -C /etc`) against the owning `fsProfile` before launch. That request-time check is not the read boundary: on Linux the child is then confined with Landlock so an allowed program cannot read a denied path through a git shell alias, `rg --pre`, or any other descendant. The ruleset handles `READ_FILE | READ_DIR | EXECUTE`, grants system roots needed to exec plus the resolved read profile (with holes punched for existing `denyRead` matches), and fails closed if Landlock is unavailable. Missing activity policy data still fails closed. [ORB-11031] [ORB-11514]
...
8. **Generic exec path admission is not the read boundary.** Activity-scoped `proc.spawn` still validates existing path arguments against the owning activity's resolved `fsProfile` before launch, but Linux children are confined with Landlock from that same profile so interpreter text inside an allowed program cannot open a denied path. CLI-agent OS wrappers remain the kernel-level boundary for the provider process itself. The program allowlist is mandatory rather than opt-in: asset load rejects an activity whose `tools` cover `proc.spawn` while `proc_allowed_programs` is absent, and the v2 activity tool context always marks `proc.spawn` activity-scoped, so a missing list denies every program instead of degrading to allow-all ([ORB-10959], [ORB-11031], [ORB-11514]).
