...
7. **The file's own ownership and mode are part of the ceiling** [ORB-12450]. A row is a grant, so write access to the file is `operator` on this machine for anyone who wants it — including a principal with no Orbit capability of its own. Load therefore refuses a file that is not a regular file owned by the effective uid, or whose mode has any of `0o022` set, naming `chmod 600 ~/.orbit/mcp-callers.toml`. The check reads ownership and mode from the opened descriptor and does not follow a final symlink, so the file that was trusted is the file that is read. `orbit mcp callers init` creates the file `0600` rather than at the ambient umask, where the conventional `umask 002` or `022` would otherwise seed a group-writable or world-readable authorization file.
8. **Group read is deliberately not required, and not refused.** The setgid Tier 2 launcher is setgid only to make Linux take its protected-exec path: it runs under the destination account's own uid, and Orbit permanently drops the launch group with `setresgid` before opening any state, so the process that reads this file is the file's owner and needs no group bit. Group and world *read* are therefore unnecessary — but they disclose the trusted machine IDs, labels, and pinned fingerprints without letting a reader raise the ceiling, so refusing them would trade a destination's remote sessions for a disclosure. `orbit doctor` reports them instead. The mode of the containing `~/.orbit` directory is a separate exposure of the same shape, tracked on its own; this check does not stand in for it.
...
5. `orbit mcp callers list` prints the default and a redacted summary for each loaded row. It reports policy shape, scope presence, and key-binding state without printing machine IDs, labels, workspace IDs, or key fingerprints because CLI output may be retained by log collectors. `orbit mcp callers check <machine_id>` prints what a session from that caller would resolve to, without serving one, and says whether the row is key-bound or selected by a name alone; identity and workspace values are redacted there as well.
...
3. `orbit doctor` reports both gaps [ORB-11053], as two rows because they are two different conditions. `mcp-callers` warns when this machine accepts SSH logins — evidenced by a non-empty `~/.ssh/authorized_keys` — and has declared nothing about who may call it; it warns when the file is readable beyond its owner [ORB-12450]; and it fails when the file exists and does not load, because every remote session is then refused — which includes a group- or world-writable file, since a ceiling anyone can add a row to is not a ceiling. `mcp-caller-keys` warns when a row grants `operator` with no `ssh_key_fingerprint`: the strongest thing the file can say, resting on a name. Both are warnings rather than errors, because Tier 2 is opt-in and a deliberate Tier 1 destination is not broken. The rows are machine-global and are composed in `orbit-cli`, since `orbit-cmd` does not know about MCP and must not learn.
