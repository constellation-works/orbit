...
| **Follower** | A replica checkout on another host running the drain in pull mode. Holds `execute`; never mints, never reserves locally, never runs an epic. [2_design.md §1](../2_design.md#1-roles-one-owner-n-followers) |
...
| **Ready queue** | Owner-maintained ordered projection of `backlog` tasks with satisfied dependencies, epics excluded, in the owner's readiness order. The only place order is decided. [2_design.md §2](../2_design.md#2-the-ready-queue-and-orbittaskpull) |
