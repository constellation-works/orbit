...
The Jobs pane combines routine targets with recent job runs. An operator session can use **Run** on a sweep or other no-input catalog job; `POST /api/jobs/:id/run?workspace=…` submits through the same runtime path as `orbit run job`, records dashboard provenance, and returns the run id and queued/submitted state. The pane shows the receipt or server error and refreshes recent runs after submission. Pausing a routine does not disable manual Run. Delivery jobs stay disabled in this no-input UI because they need a task id or delivery window; use Ship or Drain. Read-only sessions see the authorization reason, and each job retains a copyable CLI command.
