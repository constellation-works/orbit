...
Store canonical task bundles under `~/.orbit/tasks/workspaces/<workspace-id>/<task-id>/`, resolve them through the registry, and make `status` an envelope field. Status-specific and terminal-month views are generated by CLI, dashboard, and indexes rather than by directory layout.
...
- Checkouts carry no task-bundle projection; task and artifact tools use canonical registry lookup.
- Cost: humans do not get a workspace-relative task tree. Terminal-state date partitioning and browsing use indexes or retention policy.
...
## Home task store with canonical registry lookup

**Recorded:** 2026-07-26 21:51:24.377578Z · [ORB-10458]

### Context

Keeping the canonical copy inside every checkout makes gitignored task data fragile. `~/.orbit` already allocates IDs and remembers workspace bindings, so it owns canonical local task storage and task tools resolve bundles through that registry.

### Decision

Treat `~/.orbit/tasks/workspaces/<workspace-id>/<task-id>/` as the canonical local bundle. Store `workspace_id` in `.orbit/config.yaml` and mandatory allocator, workspace-binding, local execution overlay, status, relation, tag, and lock/index metadata under `~/.orbit/tasks/index.sqlite`.

### Consequences


- Task artifacts remain available through task and artifact tools without making the checkout the canonical store.
- Allocation and workspace resolution are durable without making every content write a dual-write operation.
- Workspace initialization, open, task mutations, reindex, import, and restore do not create checkout task links.
- Sync and hosted modes can replace or augment allocation without changing the workspace bundle shape.
- Cost: `.orbit/config.yaml` becomes load-bearing for binding. If it is lost, Orbit must rebind by path/repo fingerprints or prompt the user.
