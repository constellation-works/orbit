...
`AuditEvent` lives in `crates/orbit-types/src/telemetry/audit_event.rs`. Rows include execution id, timestamp, command/subcommand, optional tool and target metadata, role, status, exit code, duration, working directory, optional argument/error/stdout/stderr fields, host, pid, and session id. Nullable MCP additions include resolved workspace, caller/process machine and display-host fields, transport, origin-session id, per-call `trace_id`, and best-effort `caller_ip`. Caller machine and IP are audit metadata, not authenticated identity. The effective MCP capability set is persisted; federated routed calls may also carry `mcp_call_id`, while v1 MCP sessions do not carry execution leases, so `lease_id` remains empty. Old rows read with null/empty additions and are not rewritten.
...
tool JSON does not. Outside that envelope the role is `unverified`. The accepting MCP
server resolves local agent/operator policy or destination-side SSH caller grants, and
the tool chokepoint enforces the resulting effective capabilities. Those capabilities
are recorded on the audit row; v1 MCP sessions do not carry execution leases.
...
`V2SqliteSink` stores one serialized event payload per row in `v2_audit_events`. `crates/orbit-core/src/runtime/audit/run.rs` is the read-side accessor after [T20260426-0709], deriving activity DAG `step.id` values from `parent_event_id` ancestry and resolving CLI stdout/stderr blob references for `orbit run logs`. After [T20260508-14], the same accessor tolerates malformed stored event JSON and missing blobs for dashboard inspection, returning partial per-step CLI invocation records with run id, event id, timestamp, step index, exit status, timeout, duration, provider, blob refs, and bounded stdout/stderr material.

After [ORB-10496] (see [Provider subprocess liveness is a separate audit event probed at read time](./4_decisions.md#provider-subprocess-liveness-is-a-separate-audit-event-probed-at-read-time)), the CLI backend also emits `CliInvocationProcess` (`cli.invocation.process`) carrying the spawned provider child's `pid` and its `pid_start_time` identity token, immediately after spawn and before the supervision loop — ordered strictly between the started and finished events, and persisted synchronously so it is readable while the invocation is still running. `OrbitRuntime::collect_run_provider_processes` pairs a process and `cli.invocation.finished` event by their shared direct `parent_event_id` invocation identity as well as their enclosing step ([ORB-11284](./4_decisions.md#provider-completions-correlate-by-invocation-ancestry)). This preserves sequential retries while keeping same-step parallel invocations distinct even when completions arrive out of order. Historical events without ancestry pair only when exactly one ancestry-free process is open; ambiguity remains open and receives a liveness probe. The accessor resolves liveness for still-open records through `orbit_common::process::identity::probe_process_liveness`, which reports `alive` / `exited` / `unknown` and treats a live PID with a disagreeing versioned identity token as `exited`. `GET /api/runs/:id` returns the projection as `provider_processes`, and `orbit run show` prints one `Agent:` line per still-open child. This is the only channel that observes ship-pipeline `agent_implement` agents: they are children of the pipeline worker, so the Worker-daemon run store behind bridge `agent_run_list` never records them.
...
`orbit.task.update` also accepts `note` on every legal status change. The note is stored on that transition's history entry as its `status_note`; it is not a general discussion field. A write with `note` but no actual status change is rejected and directs the caller to `comment`, which remains the append-only free-form discussion surface. Guarded approval and pickup transitions retain their specialized behavior while sharing the same history annotation contract.
...
The audit CLI exposes command rows through `orbit audit list`, `show`, `stats`, `export --format json`, `export --format csv`, and `prune`. Additive fields and filters cover workspace, caller/process machine, transport, origin session, canonical run, and compatibility capability/call/lease columns. JSON, CSV, show output, and `orbit-web` projections preserve the nullable schema. Current MCP rows additionally expose a per-call `trace_id` and optional observed `caller_ip`; effective capability filters match the persisted session grants, federated routed calls may populate `mcp_call_id`, and lease filters match nothing because v1 MCP sessions do not carry execution leases.
...
After [T20260510-13] and [ORB-00062], friction reporting is outside the task lifecycle: `orbit.friction.add`, `list`, `show`, `tags`, `update`, and `resolve` expose scan and triage helpers backed by the workspace-partitioned host-global store, while `orbit.friction.stats` computes `open`, `triaged`, `resolved_this_month`, total resolved count, and model/tag rates on demand. The legacy `.orbit/frictions/` tree is imported as rollback evidence rather than used as the live source. After [ORB-10202], `friction` is no longer a `TaskStatus` variant or accepted persisted task value. The dashboard `Knowledge > Frictions` subtab delegates to the same tool helpers through `/api/frictions*`, so human triage and CLI/MCP reads share one vocabulary and stats shape.
...
The friction taxonomy remains operator-owned and closed: unknown tags are rejected with the valid vocabulary. Agent-facing writes normalize common category spellings before that validation (`ci`/`test`/`tests`/`testing` → `build`, `cli`/`mcp`/`tool` → `tooling`, `auto-task`/`auto-tasks`/`scheduler`/`routine` → `automation`, and `skill`/`prompt` → `skill-guidance`) and report each substitution in `tag_normalizations`. In a workspace-bound MCP session, `tools/list` advertises the live `tags.yaml` names and descriptions; an unbound session advertises the shipped defaults and states that a workspace may extend them. This keeps aggregation canonical without making callers guess the category vocabulary.

After [ORB-10590], a record carries an author-settable `title` — its handle in `friction list`, the dashboard, and the pre-filing search that decides whether a problem is already known. `orbit.friction.add` accepts it and `orbit.friction.update` can retitle a record without touching its append-only body. Callers that supply none get one derived from the body at write time, so the frontmatter always states the handle rather than leaving every reader to re-derive it; records written before the field existed carry no `title` and derive one on read instead, which is why no migration pass is owed. Derivation is structural (`orbit_common::governance::friction::title`): a leading heading is the record's own title only when no sibling heading at its level or shallower follows it, a leading bold run is an inline lead-in whose sentence is the subject, and the result is clamped to `FRICTION_TITLE_MAX_CHARS`. There is no separate `summary` field — [Friction records carry an author-settable title; derivation is a structural fallback](./4_decisions.md#friction-records-carry-an-author-settable-title-derivation-is-a-structural-fallback) records why the record keeps exactly one short handle.
