...
- **orbit-automation**: the scheduling domain for routines, auto-tasks and delivery/state automation [ORB-11330, ORB-12262]. Owns definition discovery/validation, due evaluation, overlap/retry coordination, coverage acceptance, the OS clock unit (`routines::clock`/`clock_unit`), the host sweep pass (`routines::tick`), the routine status projection, the auto-task CRUD surface, and the per-consumer composition in `consumers` that drives the shared evaluators. Every host capability it needs — identity, paths, the automation store, task and run lifecycle, submission, authorization, provider CLI arguments — arrives through the `host::AutomationHost` port that `orbit-core` implements for `OrbitRuntime` in `adapter/automation_host`; the crate names no runtime type. State consumers [ORB-11331] own material fingerprints and causal incident rules in `members`; the host supplies bounded task/run/source facts and adapts existing pilot/triage prepare and apply. State members reuse the Store consumer/coverage transaction and action-key admission, with no host-side checkpoint or eligibility copy; Store owns cursors, claims and receipts. Depends on Store, Common, Types, and `orbit-config` for the host-local scheduler database path alone; never Core, Engine or Registry. No ticking loop and no independent persistence store. Operation mode [ORB-11332] supplies the evaluator resolved `MemberConstraints` (grant scope and due interval) through the port; the evaluator, fingerprint, and checkpoint paths are not duplicated, and Automation never reads grants. The `review` module [ORB-11333] owns the shared before-PR coverage rules (task-meaning digest, certificate acceptance, exact-tree exclusion, landing classification); the delivery evaluator applies exclusions for review consumers only, proven through the port. Automation never persists review evidence.
- **orbit-core** `adapter/automation_host` [ORB-12262]: the single implementation of `orbit_automation::host::AutomationHost` for `OrbitRuntime`, plus the thin `runtime.auto_task_*` facade over `orbit_automation::auto_tasks::crud`. It translates and delegates only: `show_job_run` keeps the runtime's stale-run reconciliation in the path Automation observes, `run_owner_liveness`, `member_constraints` and `review_exclusions` forward to the owning Core modules, and `provider_pull_requests_argv` keeps `orbit-tools` knowledge on the Core side of the port. No scheduling decision is made here.
...
- **orbit-cmd**: shared application composition for CLI and Web consumers. It owns CLI-facing command groups plus registry-aware runtime and routine assembly, joining `orbit-core` kernels and the `orbit-automation` scheduler entry points to `orbit-registry` without reversing either lower-layer dependency. `update` is the one group that composes outward instead of downward: it owns install-channel detection, release download and integrity, executable replacement, and the post-replacement convergence the *newly installed* binary performs as a subprocess. Runtime methods are exposed as per-module `*Commands` extension traits.
- **orbit-cli**: clap-based entry point and local client-configuration surface. It assembles MCP, Registry, Web, Core, and — for the sweep, routine, clock and auto-task command groups — `orbit-automation` directly. `mcp serve` and `mcp listen` compose one host and serve it over stdio or TCP; `mcp serve --mode remote` delegates only the byte-transparent SSH process to `orbit-mcp`. In every case the accepting machine resolves local state and dispatches through Core.
