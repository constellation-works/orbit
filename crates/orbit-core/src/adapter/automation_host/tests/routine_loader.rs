...
//! both sources.
//!
//! These drive `collect_workspace_routines` over a real seeded source workspace (the same
...
use orbit_automation::routines::loader::{
    LoadedRoutine, RoutineCollection, RoutineOrigin, collect_workspace_routines,
};
...
fn collect(ws: &SourceWorkspace) -> RoutineCollection {
    collect_workspace_routines(&[(ws.workspace.clone(), ws.runtime.clone())])
}
