...
use chrono::Utc;
use orbit_automation::consumers::{members::StateConsumer, preparation};
use orbit_automation::members::{MemberAdmission, MemberHost};
use orbit_automation::source::{ls_tree_invocations, reset_ls_tree_invocations};
...
fn preparation_page_lists_instructions_once_for_all_eligible_tasks() {
    ...
    let page = StateConsumer::new(&runtime, &trigger)
...
fn stale_membership_between_observe_and_admission_is_retired() {
    ...
    let trigger = trigger();
    let consumer = StateConsumer::new(&runtime, &trigger);
    let page = consumer.observe(None, Utc::now()).unwrap();
    ...
    assert_retire(
        consumer.admission(&member).unwrap(),
...
fn stale_recovery_cannot_authorize_and_material_changed_still_fires() {
    ...
    assert_retire(
        consumer.admission(&stale_fingerprint).unwrap(),
        "material_changed",
    );
    ...
    match consumer.admission(&member).unwrap() {
...
fn multi_candidate_admission_reuses_inventory_after_freshness_check() {
    ...
    let page = consumer.observe(None, Utc::now()).unwrap();
    assert_eq!(page.candidates.len(), 3);
    let after_observe = consumer.incident_work_stats();
    assert_eq!(
    ...
    for candidate in &page.candidates {
        assert!(matches!(
            consumer.admission(candidate).unwrap(),
            MemberAdmission::Admit
        ));
    }

    let stats = consumer.incident_work_stats();
