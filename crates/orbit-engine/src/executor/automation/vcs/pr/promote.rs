...
use super::super::commit::{verified_clean_tree_checkpoint, verify_clean_tree_handoff};
...
pub(in crate::executor::automation) fn pr_promote<H: RuntimeHost + ?Sized>(
    ...
    let pr_number = if no_diff_expected {
        if let Some(checkpoint) = verified_clean_tree_checkpoint(input) {
            verify_clean_tree_handoff(
                host,
                &context.tasks,
