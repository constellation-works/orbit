...
//!
//! Every observation runs as a child process under one wall-clock and output
//! budget, so an unavailable or pathological repository defers instead of
//! hanging a scheduler pass. Provider (pull-request) identity is fetched
//! through the host's provider CLI arguments, never composed here.
...
/// Reset the instruction-scan counter before an observation under test.
#[cfg(any(test, feature = "test-support"))]
pub fn reset_ls_tree_invocations() {
    LS_TREE_INVOCATIONS.with(|count| count.set(0));
}

/// How many `git ls-tree` scans ran since the last reset, so a caching change
/// is asserted on work performed rather than on output alone.
#[cfg(any(test, feature = "test-support"))]
pub fn ls_tree_invocations() -> usize {
    LS_TREE_INVOCATIONS.with(std::cell::Cell::get)
}

/// Bounded Git and provider observation rooted at one checkout. One value is
/// one observation: its budget starts when it is constructed.
pub struct Source<'a> {
    root: &'a Path,
    started: Instant,
}
...
impl<'a> Source<'a> {
    /// Start an observation against the checkout at `root`.
    ...
    /// Run one `git` command under this observation's budget, returning its
    /// trimmed stdout.
    pub fn git(&self, args: &[&str]) -> Result<String, AutomationError> {
        self.command("git", args)
    }

    /// Resolve `spec` to its exact commit and tree.
    pub fn revision(&self, spec: &str) -> Result<SourceRevision, AutomationError> {
        let commit = self.git(&[
            "rev-parse",
            "--verify",
            "--end-of-options",
    ...
    /// The repository identity and the configured branch's head revision —
    /// never the executor worktree's HEAD.
    pub fn head(&self, branch: &str) -> Result<(String, SourceRevision), AutomationError> {
    ...
    pub fn repository(&self) -> Result<String, AutomationError> {
        let remote = self
    ...
    pub(crate) fn observe<H: AutomationHost>(
        &self,
        host: &H,
        branch: &str,
        state: &AutomationState,
    ) -> Result<SourcePage, AutomationError> {
        self.observe_with_lookup(branch, state, &self.provider_lookup(host))
    }

    /// Ask the host which provider command lists a commit's pull requests, and
    /// run it under this observation's budget.
    pub(crate) fn provider_lookup<'h, H: AutomationHost>(
        &'h self,
        host: &'h H,
    ) -> impl Fn(&str, &str) -> Result<String, AutomationError> + 'h {
        move |repository, sha| {
            let argv = host.provider_pull_requests_argv(repository, sha)?;
            self.command("gh", &argv.iter().map(String::as_str).collect::<Vec<_>>())
        }
    }

    /// One bounded observation page with an explicit provider lookup, so
    /// callers without a host (and tests) can supply their own.
    ...
    fn observe_with_lookup_limit(
        ...
        for sha in &candidates {
            ...
            let association = match matching.as_slice() {
                [] => None,
                [pr] => Some(provider::association(pr, &repository, branch)?),
                _ => {
        ...
        let deliveries =
            provider::group(self, state, &repository, branch, &commits, &associations)?;
    ...
    pub fn replay_history_with_lookup(
        &self,
        branch: &str,
        state: &AutomationState,
        lookup: &dyn Fn(&str, &str) -> Result<String, AutomationError>,
    ...
    pub(crate) fn replay_history<H: AutomationHost>(
        &self,
        host: &H,
        branch: &str,
        state: &AutomationState,
    ...
    pub(crate) fn retain_batch(&self, batch: &CoverageBatch) -> Result<(), AutomationError> {
        let prefix = format!(
    ...
    pub(crate) fn verify_batch(&self, batch: &CoverageBatch) -> Result<(), AutomationError> {
...
/// Refuse a replay whose orphan or canonical range exceeds the traversal bound.
pub fn validate_replay_range_lengths(
    orphan_count: usize,
...
/// The single forward-ordered canonical commit a replayed orphan maps to.
/// Anything else is ambiguous and refuses.
pub fn unique_mapping_candidate(
