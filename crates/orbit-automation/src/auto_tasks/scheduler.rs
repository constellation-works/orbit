//! Auto-task scheduling policy with an explicit clock and host lifecycle
//! adapter.
//!
//! [`run_auto_task_scheduler_at`] is the host-facing pass. The policy itself is
//! written against the narrower [`AutoTaskDispatch`] seam
//! ([`run_auto_task_scheduler_with_dispatch`]), so fire / dedupe / cursor
//! behavior is exercised without a host.
...
use orbit_types::task::{Task, TaskAddParams, TaskStatus};
use orbit_types::workflow::{
    AutoTaskCursor, AutoTaskDefinition, AutoTaskPendingClaim, DedupePolicy, auto_task_tag,
};
...
/// Lifecycle adapter. The host retains task creation, authorization and audit.
...
pub fn run_auto_task_scheduler_with_dispatch(
...
/// The host's own lifecycle behind the narrower dispatch seam the policy is
/// written against.
struct HostDispatch<'a, H>(&'a H);

impl<H: AutomationHost> AutoTaskDispatch for HostDispatch<'_, H> {
    fn evaluate_delivery(
        &self,
        definition: &AutoTaskDefinition,
        dry_run: bool,
        ...
        consumers::evaluate_auto_task(self.0, definition, dry_run, now)
    }

    fn definition_root(&self) -> PathBuf {
        self.0.local_orbit_dir()
    }
    ...
    fn has_open_instance(
        ...
        open_auto_task_instance(self.0, definition)
    }

    fn mint_task(&self, definition: &AutoTaskDefinition) -> Result<String, OrbitError> {
        mint_task(self.0, definition).map(|task| task.id)
    }
}

/// Run one scheduler pass over `host`'s workspace at an explicit `now`.
pub fn run_auto_task_scheduler_at<H: AutomationHost>(
    host: &H,
    now: DateTime<Utc>,
    options: SchedulerOptions,
) -> Result<AutoTaskSchedulerOutcome, OrbitError> {
    run_auto_task_scheduler_with_dispatch(&HostDispatch(host), now, options)
}
...
pub fn open_auto_task_instance<H: AutomationHost>(
    host: &H,
    definition: &AutoTaskDefinition,
) -> Result<Option<String>, OrbitError> {
    let tasks = host.list_tasks_by_tags(&[auto_task_tag(&definition.name)])?;
...
/// dedupe: it needs only the definition, so the manual mint path
/// ([`super::crud::mint`]) reuses it verbatim and a manually minted task is
/// field-for-field identical to a fired one, provenance tag and
/// `system_created` marker included.
pub(super) fn mint_task<H: AutomationHost>(
    host: &H,
    definition: &AutoTaskDefinition,
) -> Result<Task, OrbitError> {
    host.add_task(template_params(definition))
}

/// The task a definition's template mints, as add-params.
