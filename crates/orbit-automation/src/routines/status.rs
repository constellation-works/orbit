...
use super::loader::{
    LoadedRoutine, RoutineLoadError, RoutineWorkspaceProvider, collect_workspace_routines,
};
use crate::consumers::inspect_routine;
...
pub fn routine_statuses_with_providers<P: RoutineWorkspaceProvider>(
    global_root: &Path,
    ...
    for routine in collection.routines {
        ...
        let automation = trigger_awaits_evaluation(&routine)
            .then(|| delivery_or_state_diagnostic(&discovered.entries, &routine, now_utc));
...
/// Delivery- and state-triggered routines report the shared evaluator's
/// diagnostic instead of a cron slot.
fn trigger_awaits_evaluation(routine: &LoadedRoutine) -> bool {
    routine.definition.trigger.deliveries_landed.is_some()
        || routine.definition.trigger.state.is_some()
}

/// Read-only diagnostic for one trigger-driven routine. A source workspace
/// this pass could not open, and an evaluation that could not read its state,
/// each report why rather than silently omitting the row.
fn delivery_or_state_diagnostic<H: AutomationHost>(
    entries: &[(Workspace, H)],
    routine: &LoadedRoutine,
    now_utc: DateTime<Utc>,
) -> serde_json::Value {
    let Some((_, host)) = entries
        .iter()
        .find(|(_, host)| host.shared_orbit_dir() == routine.source_orbit_dir)
    else {
        return serde_json::json!({"reason": "source_unavailable"});
    };

    match inspect_routine(host, &routine.definition, now_utc) {
        Ok(diagnostic) => serde_json::json!(diagnostic),
        Err(error) => {
            serde_json::json!({"reason": "state_unavailable", "error": error.to_string()})
        }
    }
}
...
/// state reports why.
pub fn next_scheduled_occurrence(cron: &str, now: &DateTime<Local>) -> Option<String> {
    parse_cron(cron)
