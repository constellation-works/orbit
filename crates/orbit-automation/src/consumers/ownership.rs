...
use crate::{automation_error_to_orbit, delivery};
...
pub(crate) fn resolve<H: AutomationHost>(host: &H, explicit: Option<&str>) -> DeliveryOwnership {
    let (owner_machine, authority) = match explicit {
        Some(owner) => (Some(owner.to_string()), OwnerAuthority::Definition),
        None => registered_owner(host),
    };

    DeliveryOwnership {
        owned_here: owner_machine
            .as_deref()
            .is_some_and(|owner| Some(owner) == host.machine_identity()),
        owner_machine,
        authority,
    }
}

/// Registry facts about the workspace this host is bound to. The workspace
...
fn registered_owner<H: AutomationHost>(host: &H) -> (Option<String>, OwnerAuthority) {
    match (
        host.workspace_owner_machine_id(),
        host.coordination_write_owner(),
    ) {
